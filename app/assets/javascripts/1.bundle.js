...
/***/ function(module, exports, __webpack_require__) {
	...
	__vue_template__ = __webpack_require__(/*! !vue-html-loader!./FoodEntries/FoodEntries.html */ 30)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var state = __webpack_require__(/*! ../../utilities/state.js */ 29);
	  ...
	  methods: {
	    ...
	    addEntry: function addEntry() {
	      ...
	      if (description) {
	        ...
	        this.fetch({
	          method: "POST",
	          url: this.fetchResources.food_entries.add,
	          data: data_to_send,
	    ...
	    loadDay: function loadDay() {
	      ...
	      state.push({}, this.fetchResources.food_entries.from_day(day));
	      var app = this;
	      this.fetch({
	        method: "GET",
	        url: this.fetchResources.food_entries.from_day(day),
	        onSuccess: function onSuccess(response) {
	    ...
	    autoComplete: function autoComplete(query) {
	      ...
	      this.fetch({
	        method: "get",
	        url: this.fetchResources.search_foods(query),
	        onSuccess: function onSuccess(response) {
	          if (response.success === false) {
	            alert("Unknown error...");
	    ...
	    formatAutoCompleteResults: function formatAutoCompleteResults(response) {
	      ...
	      if (response.results.length > 0) {
	        ...
	        this.fn.forEach(response.results, function (result, i) {
	          var resource = "offset" in result && "group" in result ? this.fetchResources.foods.pull(result.ndbno) : this.fetchResources.foods.show(result.id);
